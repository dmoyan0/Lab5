...
func merge(s *server) error {
	...
	// Extraer información del propio LogF3
	content, err := ioutil.ReadFile("LogF3.txt")
	if err != nil && !os.IsNotExist(err) {
		log.Printf("Error al obtener el archivo de log F3: %v\n", err)
		return err
	}
	if err == nil {
		lines := splitContentIntoLines(content)
	...
	// Actualizar el reloj vectorial f3 con el reloj combinado
	s.mu.Lock()
